...
var play = function (board, currentPlayer, symbol) {
	...
	if (currentPlayer === "Neo") {
		...
		if (checkPlay(board, xInt, yInt)) {
			...
			document.getElementById('alertOcupado').innerHTML = "Neo esta nervioso y se equivoco";
...
var main = function () {
	cleanBoard()
	var userName = "Alex";
	var computerName = "Neo";
	var board = [["[ ]", "[ ]", "[ ]"], ["[ ]", "[ ]", "[ ]"], ["[ ]", "[ ]", "[ ]"]];
	var winner = game(board, userName, computerName);
	if (winner === userName) {
		document.getElementById("winnerText").innerHTML = "El ganador es: Alex";
	} else if (winner === computerName) {
		document.getElementById("winnerText").innerHTML = "El ganador es: Neo";
	} else {
